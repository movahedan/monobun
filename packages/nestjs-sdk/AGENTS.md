...
Wrap the app with [`FetcherSettingsProvider`](../http/AGENTS.md) from `@packages/http/react` **before** using generated SWR hooks. The client mutator uses the default `fetcher` from `@packages/http`; apps merge auth via `initialSettings` / `setSettings` (`refreshConfig`, `attachAccessToken`).
...
Health / anonymous calls can use the mutator default (`publicServerClient`, `mode: 'static'`).
